...
async function getCodeQLForCmd(cmd, checkVersion) {
    const codeql = {
        ...
        async databaseInitCluster(config, sourceRoot, processName, processLevel, featureFlags, logger) {
            ...
            logger === null || logger === void 0 ? void 0 : logger.info(`Generated config at ${configLocation}`);
            if (configLocation) {
                logger === null || logger === void 0 ? void 0 : logger.info(fs.readFileSync(configLocation, "utf-8"));
...
async function generateCodescanningConfig(codeql, config) {
    var _a;
    if (!util.useCodeScanningConfigInCli() ||
        !(await util.codeQlVersionAbove(codeql, exports.CODEQL_VERSION_CONFIG_FILES))) {
        return;
    }
